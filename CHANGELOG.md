...
- deploy: `KNITWEB_BASE`-basepad — build prefixt élk absoluut asset-/pagina-pad (en de client-fetches via `window.__BASE__`) zodat de field-kit óók onder een project-subpad live kan (bv. `knitweb.github.io/knitweb/`) zonder dode links; leeg by default → root-deploy ongewijzigd. Ontgrendelt een github.io-fallback zolang `knitweb.art` geschorst is. 2 guard-tests (subpad volledig geprefixt, root ongewijzigd). Refs #9.
- KW-010: mirrors + nachtrapport + devlog-feed + timelapse — `ops/workflows/mirrors.yml` (GitLab/anchor-git + Radicle-fallback via anchor-cron), `tools/nightly-report.mjs` (rapport uit échte git-feiten, geen model-samenvatting), `feed.json` uit CHANGELOG in de build + hub-devlog-strip, `reports/render.sh` (ffmpeg-slideshow → mp4 uit ≥2 nachten). Refs #10.
